...
//fetch will get execute fast because the fetch will be in high priority queue
...
async function getData() {
    try {
        let dataFromURL = await fetch('https://fakestoreapi.com/products')
...
//i can send the data to the api
//this code is same for put as well

const addingData = {
    title: "karvalu",
    price: 200,
    description: "it a book in kannada",
    category: "novel",
    rating: {
        rate: 4.8,
        count: 300
...
//the api is mock api designed only for testing requests.
fetch('https://fakestoreapi.com/products', {
    //these are the otional parameter
    method: 'post',
    headers: { 'Content_Type': 'application/json' },
    body: JSON.stringify(addingData)   //it will conver the data into json string   
})
    .then(response => response.json())
    .then(result => console.log(result))
    .catch(error => console.log("error occured while posting the data", error))
